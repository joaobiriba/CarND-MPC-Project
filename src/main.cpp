    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double psi = j[1]["psi"];
          double v = j[1]["speed"];
          //Translate the coordinates from Map to Car system
          ...
          // Start the MPC solving
          Solution sol = mpc.Solve(state, coeffs);

          double steer_value = sol.steeringAngle.at(latency_ind);
          double throttle_value= sol.acceleration.at(latency_ind);
          ...
          msgJson["steering_angle"] = -steer_value / ( 25*M_PI / 180.0 );
          msgJson["throttle"] = throttle_value;

          msgJson["mpc_x"] = sol.positionX;
          msgJson["mpc_y"] = sol.positionY;
